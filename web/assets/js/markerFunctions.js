...
function getIconForMarkerParamType(jsonObject){
  ...
  if ( jsonObject.style == "thumbnail" ) {
    ...
    if ( !(fileExists(iconUrl)) ) {
      iconUrl = "clusteringAPI_clientServer/icon/thumb-nophoto.jpg";
    }
  ...
  if ( jsonObject.style == "marker1" ) {
     if ( jsonObject.clusterSize > 1 ) {
       iconUrl = "clusteringAPI_clientServer/icon/marker1_cluster_"+jsonObject.color+".png";
       size = new google.maps.Size(jsonObject.width, jsonObject.height);
       origin = new google.maps.Point(0,0);
       ...
       iconUrl = "clusteringAPI_clientServer/icon/marker1_single_"+jsonObject.color+".png";
       size = new google.maps.Size(jsonObject.width, jsonObject.height);
       origin = new google.maps.Point(0,0);
       anchor = new google.maps.Point(jsonObject.width/2, jsonObject.height);
       scaledSize = new google.maps.Size(jsonObject.width, jsonObject.height);
...
markerX.prototype.createInfoWindow=function()
{
  ...
  if ( !(fileExists(photourl)) )
    photourl = "clusteringAPI_clientServer/icon/nophoto.jpg";
...
markerX.prototype.getPhotoInfoInCell = function(minLat, maxLat, minLon, maxLon, k, dataSize)
{
  ...
  query = clusteringAPIPath+'markerClustering.php?type='+type
...
Label.prototype.draw = function() {
  ...
  if ( this.markerStyle == "thumbnail" ) {
    div.style.left = position.x + this.marker.width/2 - widthNumber/2 + 'px'; // 22 is the width of label
    div.style.top = position.y - this.marker.height - 22/2 + 'px'; // 22 is the height of label
  } else {
    div.style.left = position.x + this.marker.width/2 - widthNumber/2 + 'px'; // 22 is the width of label
    div.style.top = position.y + 22/2 + 'px'; // 22 is the height of label
  }
  ...
  switch ( this.markerStyle ) {
    case "thumbnail":
      if ( this.thumb!=undefined && this.clusterSize > 1 ) {
        ...
        div.innerHTML = '<img src="'+'./clusteringAPI_clientServer/icon/'+icon_filename+'" style="cursor: pointer; z-index:1000;position: absolute; width:'+width+';height:22px; " id="'+id+'" />'+ '<span id="P'+id+'" style="display:table-cell;vertical-align:middle;margin:auto;cursor: pointer; z-index:1001;overflow: hidden;position:relative;text-align: center;width:'+ width +';height: 22px;">'+nr+'</span>'; 
    ...
    case "marker1":
      var nr = this.clusterSize > 99 ? "99+" : this.clusterSize;
      div.innerHTML = '<span style="display:table-cell;vertical-align:middle;margin:auto;ursor: pointer; z-index:1000;overflow: hidden;position: relative;text-align: center;width: 30px;">'+nr+'</span>';
    break;
  }
}
